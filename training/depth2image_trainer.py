...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Path to pretrained vae from huggingface.co/models.",
    )
...
def main():
    ...
    with accelerator.main_process_first():
        ...
        #                                                               logger=logger)
        (train_loader,test_loader), dataset_config_dict = prepare_dataset(gt_datapath=args.gt_datapath,
                                                                          rgb_datapath = args.rgb_datapath,
                                                                          train_rgb_list = args.train_rgb_list, 
                                                                          train_detpthgt_list = args.train_depth_list,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with accelerator.accumulate(unet):
                ...
                # left_disparity_resized = resize_max_res_tensor(left_disparity,is_disp=False) # not range
                ...
                h_rgb = vae.encoder(left_image_data.to(weight_dtype))
                moments_rgb = vae.quant_conv(h_rgb)
                mean_rgb, logvar_rgb = torch.chunk(moments_rgb, 2, dim=1)
                rgb_latents = mean_rgb *rgb_latent_scale_factor    #torch.Size([1, 4, 54, 96])(wrong)
                
                # encode disparity to lantents
                # TODO: vae.quant_conv??
                h_disp = vae.encoder(left_disparity.to(weight_dtype))
